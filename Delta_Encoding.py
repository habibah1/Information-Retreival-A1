        ...
        with open(f,encoding="utf8",errors='ignore') as file:
            ...
            if term in data2:
                if (b==0):
                    ...
                    docids_plus_their_positions.append(docdid-olddocid)
                    olddocid=docdid
                for m in re.finditer(term,data2): 
                    if (a == 0):
                        oldelement=m.start()
                        i_am_the_sorter.append(oldelement)
                        a=1
                    else:
                        i_am_the_sorter.append(m.start()-oldelement)
                        oldelement=m.start()
                a=0
                docids_plus_their_positions.append(i_am_the_sorter) 
        docdid = docdid + 1
    b=0
